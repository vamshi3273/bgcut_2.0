...
import { useUpscale } from './use-upscale';

const UpscaleImage = ({ plans }: { plans: Plan[] }) => {
  const {
    setFile,
    previewImage,
    superUpscale,
      ...
      {previewImage ? (
        <div className="bg-accent flex flex-col rounded-4xl p-2 md:h-[500px] md:flex-row">
          ...
          <div className="bg-background flex w-full flex-col items-start gap-6 rounded-[24px] p-6 md:w-[40%]">
            <h2 className="text-xl font-semibold">Upscale Image</h2>
            <div className="mt-4 flex w-full items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <Label htmlFor="super-erase">Super Upscale</Label>
                <div className="rounded-full bg-gradient-to-r from-indigo-600 to-pink-600 px-2 py-0.5 text-xs font-semibold text-white">
                  Pro
                </div>
              </div>
              <Switch id="super-erase" checked={superUpscale} onCheckedChange={setSuperUpscale} />
            </div>
